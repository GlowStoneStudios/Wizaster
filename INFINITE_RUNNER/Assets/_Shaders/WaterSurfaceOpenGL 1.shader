...
////// Lighting:
                float attenuation = 1;
                float node_940 = pow(1.0-max(0,dot(normalDirection, viewDirection)),_ReflectionFresnel);
                float3 node_6367 = (_ReflectionColor.rgb*node_940);
                float4 _Reflection_var = texCUBE(_Reflection,viewReflectDirection);
                float3 node_9123 = (_Reflection_var.rgb*_ReflectionIntensity);
                float node_6995 = 0.0;
                float node_9304 = 1.0;
                float3 finalColor = (saturate((1.0-(1.0-saturate((1.0-(1.0-((lerp(_ShallowColor.rgb,_DepthColor.rgb,saturate((sceneZ-partZ)/_WaterDepth))*sceneColor.rgb)+(pow(max(0,dot(viewReflectDirection,lightDirection)),exp((_Gloss*9.0+1.0)))*(_LightColor0.rgb*attenuation)*_Specular)))*(1.0-lerp( node_6367, saturate((1.0-(1.0-node_6367)*(1.0-node_9123))), _UseCubemap )))))*(1.0-(_FoamColor.rgb*(1.0 - saturate((sceneZ-partZ)/_FoamBlend))*(_BorderContrast + ( (saturate((dot(node_6883,float3(0.3,0.59,0.11))*33.0)) - node_6995) * (node_9304 - _BorderContrast) ) / (node_9304 - node_6995))))))+(1.33*_FoamColor.rgb*(1.0 - saturate((sceneZ-partZ)/(_FoamBlend*0.2)))));
                fixed4 finalRGBA = fixed4(lerp(sceneColor.rgb, finalColor,saturate((sceneZ-partZ)/_EdgeBlending)),1);
...
////// Lighting:
                ...
                float node_9304 = 1.0;
                float3 finalColor = (saturate((1.0-(1.0-saturate((1.0-(1.0-((lerp(_ShallowColor.rgb,_DepthColor.rgb,saturate((sceneZ-partZ)/_WaterDepth))*sceneColor.rgb)+(pow(max(0,dot(viewReflectDirection,lightDirection)),exp((_Gloss*9.0+1.0)))*(_LightColor0.rgb*attenuation)*_Specular)))*(1.0-lerp( node_6367, saturate((1.0-(1.0-node_6367)*(1.0-node_9123))), _UseCubemap )))))*(1.0-(_FoamColor.rgb*(1.0 - saturate((sceneZ-partZ)/_FoamBlend))*(_BorderContrast + ( (saturate((dot(node_6883,float3(0.3,0.59,0.11))*33.0)) - node_6995) * (node_9304 - _BorderContrast) ) / (node_9304 - node_6995))))))+(1.33*_FoamColor.rgb*(1.0 - saturate((sceneZ-partZ)/(_FoamBlend*0.2)))));
                fixed4 finalRGBA = fixed4(finalColor * saturate((sceneZ-partZ)/_EdgeBlending),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
